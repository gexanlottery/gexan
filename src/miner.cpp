...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setGexSchedule(gexDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = gexDGP.getBlockSize(nHeight);
    minGasPrice = gexDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = gexDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    GexTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractGexTX resultConverter;
    if(!convert.extractionGexTransactions(resultConverter)){
    ...
    for(GexTransaction gexTransaction : gexTransactions){
        txGas += gexTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + gexTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(gexTransaction.gasPrice() < minGasPrice){
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    // Found a solution (stake)
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GEXMiner : generated block is stale");

        for(const CTxIn& vin : pblock->vtx[1].vin) {
            if (wallet.IsSpent(vin.prevout.hash, vin.prevout.n)) {
                return error("GEXMiner : Gen block stake is invalid - UTXO spent");
            }
        }
    }
