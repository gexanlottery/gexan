...
const int LAST_HEIGHT_FEE_BLOCK = 1180000;
...
std::string SCVersion ("/Gexan:1.0.0/");
...
CAmount GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight)
{
    ...
    if (nHeight < 7000) {
        nSubsidy = 1 * COIN;
    } else if (nHeight >= 7000 && nHeight < 50000) {
        nSubsidy = 6 * COIN;
    } else if (nHeight >= 50000 && nHeight < 180000) {
        nSubsidy = 8 * COIN;
    } else if (nHeight >= 180000 && nHeight < 310000) {
        nSubsidy = 5 * COIN;
    } else if (nHeight >= 310000 && nHeight < 440000) {
        nSubsidy = 2 * COIN;
    } else if (nHeight >= 440000) {
        nSubsidy = 1 * COIN;
    } else {
...
//#if 0
      if (pindex->nHeight > Params().FirstSCBlock()) {
        globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // gex
        globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // gex
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const CBlock* pblock)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        if (!rv) {
            ...
            if (pindexNew->nHeight >= chainparams.FirstSCBlock()) {
                globalState->setRoot(oldHashStateRoot); // gex
                globalState->setRootUTXO(oldHashUTXORoot); // gex
