...
// Copyright (c) 2018-2019 The Gexan developers
// Copyright (c) 2019 The XDNA Creation Team
...
#if defined(NDEBUG)
#error "GEX cannot be compiled without assertions."
#endif
...
std::unique_ptr<GexState> globalState = nullptr;
...
uint256 bnProofOfStakeLimitV2 = (~uint256(0) >> 20);
...
const string strMessageMagic = "Gex Signed Message:\n";
...
bool GetCoinAge(const CTransaction& tx, const unsigned int nTxTime, uint64_t& nCoinAge)
{
    ...
    if (!hideLogMessage && bnCoinDay != uint256(0))
...
#                   endif
        ...
        if(chainActive.Height() >= chainparams.FirstSCBlock() && tx.HasCreateOrCall()) {
            ...
            GexDGP gexDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = gexDGP.getMinGasPrice(chainActive.Height() + 1);
            uint64_t blockGasLimit = gexDGP.getBlockGasLimit(chainActive.Height() + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            GexTxConverter converter(tx, NULL);
            ExtractGexTX resultConverter;
            if(!converter.extractionGexTransactions(resultConverter)){
            ...
            std::vector<GexTransaction> gexTransactions = resultConverter.first;
            std::vector<EthTransactionParams> gexETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(GexTransaction gexTransaction : gexTransactions){
                sumGas += gexTransaction.gas() * gexTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, gexTransaction.gasPrice());
                } else {
                    txMinGasPrice = gexTransaction.gasPrice();
                }
                VersionVM v = gexTransaction.getVersion();
                if(v.format!=0)
                ...
                if(gexTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(gexTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(gexTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)gexTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(gexETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > gexTransactions.size())
...
CAmount GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    ...
    } else if (nHeight <= 50000 && nHeight >= 2) {
        nSubsidy = 0.6 * COIN;
    } else if (nHeight <= 70000 && nHeight >= 50001) {
        nSubsidy = 9.6 * COIN;
    } else if (nHeight <= 200000 && nHeight >= 70001) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <= 460000 && nHeight >= 200001) {
        nSubsidy = 3.6 * COIN;
    } else if (nHeight >= 460001) {
        nSubsidy = 2.4 * COIN;
    } else {
        nSubsidy = 0.5 * COIN;
    }
...
#if 0
    GexDGP gexDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setGexSchedule(gexDGP.getGasSchedule(pindex->nHeight + 1));
#endif
    uint64_t minGasPrice = 0;//gexDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = DEFAULT_BLOCK_GAS_LIMIT_DGP;//gexDGP.getBlockGasLimit(pindex->nHeight + 1);

#if 0
    uint32_t sizeBlockDGP = 0;//gexDGP.getBlockSize(pindex->nHeight + 1);
        ...
        if (fAddressIndex) { // OUTPUTS
            for (size_t k = 0; k < tx.vout.size(); k++) {
                ...
                if (ExtractDestination(out.scriptPubKey, dest, &txType)) {
                    ...
                    // outputs like OP_RETURN custom data, some GEX v3 vouts had 1 satoshi in nValue
...
///////////////////////////////////////////////////////////////////////////////////////// gex
        ...
        if (pindex->nHeight >= Params().FirstSCBlock()) {
            ...
            if(tx.HasCreateOrCall() && !hasOpSpend) {
                ...
                GexTxConverter convert(tx, &view, &block.vtx);

                ExtractGexTX resultConvertGexTX;
                if(!convert.extractionGexTransactions(resultConvertGexTX)) {
                ...
                if(!CheckMinGasPrice(resultConvertGexTX.second, minGasPrice))
                ...
                dev::u256 gasAllTxs = dev::u256(0);
                ByteCodeExec exec(block, resultConvertGexTX.first, blockGasLimit);
                ...
                for(GexTransaction& ltx : resultConvertGexTX.first) {
                ...
                {
                    for(size_t k = 0; k < resultConvertGexTX.first.size(); k ++){
                        ...
                        tri.push_back(TransactionReceiptInfo{block.GetHash(pindex->nHeight >= Params().SwitchPhi2Block()), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertGexTX.first[k].from(), resultConvertGexTX.first[k].to(),
...
bool CheckForMasternodePayment(const CTransaction& tx, const CBlockHeader& header) {
    ...
    // Divide to keep a check precision of 0.01 GEX
    ...
    if (tx.IsCoinBase()) {
        roundMnAward =  GetMasternodePoWReward(nHeight,totalReward);
    }
...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, CNode* pfrom, const CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nHeight >= nGexProtocolSwitchHeight)
    {
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if ((coins.DynamicMemoryUsage() + pcoinsTip->DynamicMemoryUsage()) > nCoinCacheUsage) {
            LogPrintf("%s: WARNING nCoinCache reached (%zu > %zu), please increase -dbcache\n", __func__,
 ...
 * As an example, Gexan uses /Gexan:X.X.X/ version string. So, this function will
 * return 5 for /Gexan:5.0.0/ or 0 if it couldn't find ":" or "." delimeters, or if no digits are between
 * those delimeters, or if string verison is empty
 * @param cleanVersion Gexan wallet version as a string
 * @return Major Gexan major version or 0
 */
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    GexTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<GexTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path gexDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(gexDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(gexDir.string(), std::ios::in | std::ios::out);
...
bool GexTxConverter::extractionGexTransactions(ExtractGexTX& gextx){
    std::vector<GexTransaction> resultTX;
    ...
    gextx = std::make_pair(resultTX, resultETP);
    return true;
}

bool GexTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool GexTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
GexTransaction GexTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    GexTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = GexTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = GexTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
