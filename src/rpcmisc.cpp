...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total gex balance of the wallet\n"
            "  \"darksend_balance\": xxxxxx, (numeric) the anonymized gex balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in gex/kB\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in gex/kB\n"
...
UniValue getstateinfo(const UniValue& params, bool fHelp)
{
    ...
    GexDGP gexDGP(globalState.get());

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("maxblocksize", (int)gexDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (int)gexDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (int)gexDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"gexaddress\"\n"
            "\nReturn information about the given gex address.\n"
            "\nArguments:\n"
            "1. \"gexaddress\"     (string, required) The gex address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"gexaddress\",     (string) The gex address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: GEX address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are gex addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) gex address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"gexaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"gexaddress\"  (string, required) The gex address to use for the signature.\n"
...
UniValue getaddressbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "}\n"
            "\nResult: {\n"
            "  \"balance\",  (number) The current balance (in GEX satoshis)\n"
...
UniValue getaddresstxids(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1)
        throw runtime_error(
            ...
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddresstxids", "\"GYmrT81UoxqfskSNt28ZKZ3XXskSFENEtg\" 0 100000")
            + HelpExampleCli("getaddresstxids", "'{\"addresses\": [\"GYmrT81UoxqfskSNt28ZKZ3XXskSFENEtg\"]}'")
            + HelpExampleRpc("getaddresstxids", "{\"addresses\": [\"GYmrT81UoxqfskSNt28ZKZ3XXskSFENEtg\"]}")
        );
