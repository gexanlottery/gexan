...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GEX);
    ...
    if (!settings.contains("nAnonymizeGexAmount"))
        settings.setValue("nAnonymizeGexAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeGexAmount = settings.value("nAnonymizeGexAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGexAmount"))
        SoftSetArg("-anonymizegexamount", settings.value("nAnonymizeGexAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in gex.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGexAmount:
            return QVariant(nAnonymizeGexAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case DarkSendRounds:
            ...
            break;
        case AnonymizeGexAmount:
            nAnonymizeGexAmount = value.toInt();
            settings.setValue("nAnonymizeGexAmount", nAnonymizeGexAmount);
            Q_EMIT anonymizeGexAmountChanged(nAnonymizeGexAmount);
            break;
        case CoinControlFeatures:
