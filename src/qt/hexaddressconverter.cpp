...
void HexAddressConverter::addressChanged(const QString& address) {
    ...
    if(!address.isEmpty()) {
        ...
        hasGexAddress = IsValidDestination(addr);
        if (hasGexAddress) {
            ui->label->setText("Address (Gex)");
        ...
        isAddressValid = hasHexAddress || hasGexAddress;
    }
