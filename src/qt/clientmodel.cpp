...
void ClientModel::getGasInfo(uint64_t& blockGasLimit, uint64_t& minGasPrice, uint64_t& nGasPrice) const
{
    ...
    blockGasLimit = gexDGP.getBlockGasLimit(chainActive.Height());
    minGasPrice = CAmount(gexDGP.getMinGasPrice(chainActive.Height()));
...
static void BlockTipChanged(ClientModel *clientmodel, bool initialSync, const CBlockIndex *pIndex, bool fHeader)
{
    ...
    if (fHeader || (pIndex->nHeight % 100) == 0) {
        // cache best headers time and height to reduce future cs_main locks
        clientmodel->cachedBestHeaderHeight = pIndex->nHeight;
        clientmodel->cachedBestHeaderTime = pIndex->GetBlockTime();

        // refresh headers, the only height shown on modal overlay (fHeader seems never true on modaloverlay)
