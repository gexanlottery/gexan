...
//    SetObjectStyleSheet(ui->pushButtonClearAll, StyleSheetNames::ButtonBlack);
    ...
    m_tabInfo->addTab(0, tr("Write to Contract"));
    ...
    if(isDataValid())
    {
        ...
        if(retval == QMessageBox::Yes) {
            ...
            if(errorMessage.isEmpty() && m_execRPCCommand->exec(lstParams, result, resultJson, errorMessage)) {
                ...
                QMessageBox::warning(this, tr("Write to Contract"), errorMessage);
            }
        }
    }
}
...
void SendToContract::on_numBlocksChanged(int newHeight)
{
    if(m_clientModel)
    {
        ...
        if (lastUpdatedHeight < newHeight) {
            ...
            ui->labelGasPrice->setToolTip(tr("Gas price: GEX price per gas unit. Default = %1, Min = %2.").arg(
