...
bool ToHash160(const std::string& strGexAddress, std::string& strHash160)
{   
    CTxDestination gexAddress = DecodeDestination(strGexAddress);
    if(IsValidDestination(gexAddress)){     
        CKeyID *keyid = boost::get<CKeyID>(&gexAddress);
...
bool ToGexAddress(const std::string& strHash160, std::string& strGexAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    
    if(IsValidDestination(CTxDestination(keyid))){
        strGexAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToGexAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToGexAddress(tokenEvent.receiver, tokenEvent.receiver);
