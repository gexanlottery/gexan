...
static void NotifyGexNodeUpdated(MasternodeManager *page, CGexNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateGexNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyGexNodeChanged.connect(boost::bind(&NotifyGexNodeUpdated, this, _1));
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CGexNodeConfig c = pwalletMain->mapMyGexNodes[sAddress];
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyGexNodes.erase(sAddress);
        walletdb.EraseGexNodeConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    for (PAIRTYPE(std::string, CGexNodeConfig) adrenaline : pwalletMain->mapMyGexNodes)
    {
        CGexNodeConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
