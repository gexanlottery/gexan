...
bool GexDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule GexDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t GexDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t GexDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t GexDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t GexDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool GexDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void GexDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void GexDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void GexDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void GexDGP::createParamsInstance(){
...
dev::Address GexDGP::getAddressForBlock(unsigned int blockHeight){
...
void GexDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void GexDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void GexDGP::parseStorageOneUint64(uint64_t& value){
...
void GexDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule GexDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
