...
GexState::GexState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = GexState::openDB(_path + "/gexDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

GexState::GexState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute GexState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, GexTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createGexAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            gex::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> GexState::vins() const // temp
{
...
void GexState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* GexState::vin(dev::Address const& _a) const
{
    return const_cast<GexState*>(this)->vin(_a);
}

Vin* GexState::vin(dev::Address const& _addr)
{
...
// void GexState::commit(CommitBehaviour _commitBehaviour)
// {
...
void GexState::kill(dev::Address _addr)
{
...
void GexState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
const dev::Address GexState::createGexAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void GexState::deleteAccounts(std::set<dev::Address>& addrs){
...
void GexState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void GexState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
