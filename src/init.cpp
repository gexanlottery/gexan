...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "gex.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "gexd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in GEX/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in GEX/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in GEX/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-anonymizegexamount=<n>", strprintf(_("Keep N GEX anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The GEX-Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The GEX Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2019 The XDNA Creation Team"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_(" ")) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that GEX is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // check for -mnsec
    fDebugMnSecurity = GetBoolArg("-mnsec", true);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Gexan is shutting down."));
...
#endif
    // Make sure only a single GEX process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE *file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file)
        fclose(file);
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Gexan is probably already running."), strDataDir));
    ...
    while (!fLoaded && !fRequestShutdown)
    {
        ...
        do
        {
            try
            {
                ...
                boost::filesystem::path gexStateDir = GetDataDir() / "stateGex";

                bool fStatus = boost::filesystem::exists(gexStateDir);
                const std::string dirGex(gexStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsGexState = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<GexState>(new GexState(dev::u256(0), GexState::openDB(dirGex, hashDB, dev::WithExisting::Trust), dirGex, existsGexState));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::gexMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(gexStateDir.string());
                if (fReset) {
...
// ********************************************************* Step 8: load wallet
    ...
    else
    {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Gexan") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Gexan to complete") << "\n";
    ...
    nAnonymizeGexAmount = GetArg("-anonymizeGexamount", 0);
    if (nAnonymizeGexAmount > 999999) nAnonymizeGexAmount = 999999;
    if (nAnonymizeGexAmount < 2) nAnonymizeGexAmount = 2;

    LogPrintf("GEX darksend rounds %d\n", nDarksendRounds);
    LogPrintf("Anonymize Gex Amount %d\n", nAnonymizeGexAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1Gex+1000 == (.1Gex+100)*10
       10Gex+10000 == (1Gex+1000)*10
    */
