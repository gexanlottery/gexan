...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "gex" is a composite category enabling all GEX-related debug output
            if (ptrCategory->count(string("gex"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Gexan.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override Gexan.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Gexan.pid"));
