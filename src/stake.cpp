...
static const int LAST_MULTIPLIED_BLOCK = 69;
...
// GEX Stake modifier used to hash the stake kernel which is chosen as the stake
...
static bool GetGexStakeKernel(unsigned int nTimeTx, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake) {
...
// Note: Separated GetKernelStakeModifier & GetGexStakeKernel for convenient in case we need any other fork later on
static bool GetKernelStakeModifier(unsigned int nTimeTx, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake) {
    return GetGexStakeKernel(nTimeTx, nStakeModifier, nStakeModifierHeight, nStakeModifierTime, fPrintProofOfStake);
}
...
#       endif
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && hashProofOfStake > bnTarget && nStakeModifierHeight < 68 && nStakeModifierHeight <= LAST_MULTIPLIED_BLOCK) {
