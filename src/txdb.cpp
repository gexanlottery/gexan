    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            if (chType == DB_BLOCK_INDEX) {
                ...
                pindexNew->hashStateRoot  = diskindex.hashStateRoot; // gex
                pindexNew->hashUTXORoot   = diskindex.hashUTXORoot; // gex
                ...
                if (isPoW) {
                    ...
                    auto const &hash(pindexNew->GetBlockHash());
                    uint256 proof;
                    if (diskindex.nHeight > 30900 && pindexNew->hashProofOfStake == 0) {
