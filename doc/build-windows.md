...
Below are some notes on how to build Gexan for Windows.

The options known to work for building Gexan on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Gexan Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Gexan applications.
...
Note that for WSL the Gexan source path MUST be somewhere in the default mount file system, for
example /usr/src/gex, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
Once the source code is ready the build steps are below.
    ...
    CONFIG_SITE=$PWD/depends/x86_64-w64-mingw32/share/config.site ./configure --prefix=/ --disable-tests CPPFLAGS="-DMINIUPNP_STATICLIB"
    make
...
way. This will install to `c:\workspace\gex`, for example:

    make install DESTDIR=/mnt/c/workspace/gex
...
causes two of the gex executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the gex code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the gex source code).
